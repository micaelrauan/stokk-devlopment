...
import { Package, ShoppingBag, AlertTriangle, XCircle, TrendingDown, Activity } from 'lucide-react';
...
export default function DashboardPage() {
  const { products, totalProducts, totalItems, lowStockCount, outOfStockCount, alerts, inventoryLogs } = useInventoryContext();
  ...
  const topMovement = useMemo(() => {
    ...
    inventoryLogs.filter(l => l.type === 'OUT').forEach(l => {
      if (!map[l.productId]) map[l.productId] = { name: l.productName, total: 0 };
  ...
  const movementHistory = useMemo(() => {
    ...
    inventoryLogs.forEach(l => {
      const day = days.find(d => isSameDay(d.dateObj, startOfDay(new Date(l.timestamp))));
      if (!day) return;
  ...
  const healthyVariants = products.reduce((s, p) => s + p.variants.filter(v => v.currentStock > p.minStockThreshold).length, 0);
  const healthPercent = totalVariants > 0 ? Math.round((healthyVariants / totalVariants) * 100) : 100;
    ...
    <div className="space-y-8">
      ...
      <div className="glass-card rounded-xl p-6">
        <div className="flex items-center gap-3 mb-4">
          ...
          <h2 className="text-lg font-heading font-semibold">Saúde do Estoque</h2>
        </div>
        <div className="flex items-center gap-6">
          <div className="relative w-24 h-24">
            <svg viewBox="0 0 100 100" className="w-full h-full -rotate-90">
              ...
              <circle
                ...
                stroke={healthPercent >= 70 ? 'hsl(var(--success))' : healthPercent >= 40 ? 'hsl(var(--warning))' : 'hsl(var(--destructive))'}
          ...
          <div className="space-y-1 text-sm">
            <p><span className="font-semibold text-success">{healthyVariants}</span> variações com estoque saudável</p>
            <p><span className="font-semibold text-warning">{lowStockCount}</span> com estoque baixo</p>
            <p><span className="font-semibold text-destructive">{outOfStockCount}</span> esgotadas</p>
            <p className="text-muted-foreground text-xs mt-2">De {totalVariants} variações totais</p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="glass-card rounded-xl p-6">
          <h2 className="text-lg font-heading font-semibold mb-4">Valor do Estoque por Categoria</h2>
        ...
        <div className="glass-card rounded-xl p-6">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h2 className="text-lg font-heading font-semibold">Top 5 — Maior Giro (Saídas)</h2>
          </div>
          {topMovement.length > 0 ? (
            ...
            <p className="text-muted-foreground text-sm text-center py-8">Sem movimentações registradas</p>
          )}
        </div>
      </div>
      ...
      <div className="glass-card rounded-xl p-6">
        <h2 className="text-lg font-heading font-semibold mb-4">Movimentações — Últimos 7 dias</h2>
        <ResponsiveContainer width="100%" height={280}>
          <LineChart data={movementHistory}>
            ...
            <Tooltip />
            <Legend />
            <Line type="monotone" dataKey="entradas" name="Entradas" stroke="hsl(var(--success))" strokeWidth={2} dot={{ r: 4 }} />
            <Line type="monotone" dataKey="saidas" name="Saídas" stroke="hsl(var(--destructive))" strokeWidth={2} dot={{ r: 4 }} />
          </LineChart>
        </ResponsiveContainer>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="glass-card rounded-xl p-6">
          ...
          <div className="space-y-3">
            {[...products]
              .sort((a, b) => {
                const totalA = a.variants.reduce((s, v) => s + v.currentStock, 0);
                const totalB = b.variants.reduce((s, v) => s + v.currentStock, 0);
                return totalA - totalB;
              })
              .slice(0, 5)
              .map(product => {
                const total = product.variants.reduce((s, v) => s + v.currentStock, 0);
                return (
