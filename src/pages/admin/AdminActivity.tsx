...
export default function AdminActivity() {
  ...
  useEffect(() => {
    const load = async () => {
      const { data } = await from("inventory_logs")
        .select("*")
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground mt-1">
          Logs de atividade recentes ({filtered.length} registros)
        </p>
      </div>
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        <div className="relative flex-1">
          ...
          <Input
            placeholder="Buscar por produto, variante ou motivo..."
            value={search}
            ...
            {filtered.map((log) => (
              <tr key={log.id} className="border-t border-border/50">
                ...
                <td className="py-2 px-4">
                  <span
                    className={`text-xs px-2 py-0.5 rounded-full ${log.type === "IN" ? "bg-success/10 text-success" : log.type === "OUT" ? "bg-destructive/10 text-destructive" : "bg-info/10 text-info"}`}
                  >
                    {log.type}
                  </span>
                </td>
                <td className="py-2 px-4 font-mono">{log.quantity}</td>
                <td className="py-2 px-4 text-muted-foreground text-xs hidden md:table-cell">
                  {log.reason}
                </td>
              </tr>
            ))}
