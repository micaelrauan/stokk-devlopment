...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      const users = (profilesRes.data ?? []).filter(
      ...
      setStats({
        ...
        revenue: (salesRes.data ?? []).reduce(
          (s: number, r: any) => s + Number(r.total),
          0,
        ),
      });
      setRecentUsers(
        users.slice(0, 5).map((p: any) => ({
          id: p.id,
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4">
        {cards.map((c) => (
          <div key={c.label} className="glass-card rounded-xl p-5">
            ...
            <p className={`text-2xl font-heading font-bold mt-1 ${c.color}`}>
              {c.value}
            </p>
          </div>
        ))}
            ...
            {recentUsers.map((u) => (
              ...
              >
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span
                    className={`text-xs px-2 py-1 rounded-full ${u.plan === "premium" ? "bg-warning/10 text-warning" : "bg-muted text-muted-foreground"}`}
                  >
                    {u.plan || "free"}
                  </span>
                </div>
